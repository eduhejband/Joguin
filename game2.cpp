...
Game2::Game2() : window(sf::VideoMode(800, 600), "Gol a Gol"), ballVelocity(0.f, 0.f), moveSpeed(300.0f), followBall(false), gameActive(true), showRetryButton(false) {
...
void Game2::update(sf::Time deltaTime) {
    if (!gameActive) return;

    float moveAmount = moveSpeed * deltaTime.asSeconds();

    // Movimentação da barra do jogador 1 (energyBar)
    ...
    // Movimentação da barra do jogador 2 (topEnergyBar) com A e D
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
        topEnergyBar.move(-moveAmount, 0);
        if (topEnergyBar.getPosition().x < 0) {
            topEnergyBar.setPosition(0, 20);
        }
    }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::D)) {
        topEnergyBar.move(moveAmount, 0);
        if (topEnergyBar.getPosition().x + topEnergyBar.getSize().x > window.getSize().x) {
            topEnergyBar.setPosition(window.getSize().x - topEnergyBar.getSize().x, 20);
        }
    }
    ...
    if (ball.getPosition().y < 0) {
        ...
        winnerTextString = "Player 1 Wins!";
    } else if (ball.getPosition().y + ball.getRadius() * 2 > window.getSize().y) {
        gameActive = false;
        showRetryButton = true;
        winnerTextString = "Player 2 Wins!";
    }
}
...
void Game2::handleBarCollision(const sf::RectangleShape& bar, bool isBottomBar) {
